...
插件支持 SMTP 发信、HTML 邮件模板、模板变量、CSV 收件人导入/导出、活动订阅列表、短代码前台订阅表单、批量发送和定时发送。后台及前台订阅界面默认使用中文。
...
插件后台和前台订阅界面目前优先保留中文。发行包不再附带翻译文件，发布到 WordPress.org 后可通过 translate.wordpress.org 管理社区翻译。
